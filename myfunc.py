...
#    try:
        ...
        if request == "salarycheck":
            ...
            prevmonth = pd.to_datetime(refmonth) + pd.DateOffset(months = -1)
            ...
            for hp in concurrent.futures.as_completed(heavyprocess):
                ...
                if res[1] == "grosscur":
                    for key, value in res[2].items():
                        resdict[key]["CurrGrossData"] = value
                    #
                elif res[1] == "grossretro":
                    for key, value in res[2].items():
                        resdict[key]["RetroGrossData"] = value
                    #
                #               
            #

            jsdict = json.dumps(resdict)
            ...
            with open(".\\drafts\\"+ refmonth.strftime("%Y-%m") +".xlsx","rb") as f:
                ...
                replymsg = json.dumps([f.name,file64dec,jsdict]).encode('UTF-8')
            #

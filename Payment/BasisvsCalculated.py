...
def BasisvsCalculated(level="0.1"):
    ...
    query = f"""
    SELECT Empid,mn,Empid_mn,Dirug,Empname,Elemtype,Elem_heb,Amount,Elem,Division 
    FROM dfcurr 
    WHERE Refdate = (SELECT MAX(Refdate) FROM dfcurr) 
    AND (Elem IN {str(tuple(semelsneeded))} OR Elem_heb = {chr(34) + custom.dayvalue + chr(34)}) 
    AND Division <> {custom.pensiondepartment} 
    AND mn <> 99 AND Rank NOT IN {str(tuple(custom.hourwageranks))}
    """
...
### Case of Stopped Employees    
    ...
    query3 = f"""SELECT DISTINCT Empid,Stopname,Stopfrom,Stoptill FROM dfcurr WHERE Stopfrom IS NOT NULL AND Empid IN {str(tuple(findf["Empid"].to_list()))}"""

    stopdf = pd.read_sql_query(query3, conn)
    
    stopdf["Stop"] = stopdf.apply(lambda row: f"{row['Stopname']} {row['Stopfrom']} - {row['Stoptill']}", axis=1)
    stopdf.drop(columns=["Stopname","Stopfrom","Stoptill"], inplace=True)

    findf["Stop"] = findf.apply(lambda row: stopdf.loc[stopdf["Empid"] == row["Empid"],"Stop"].unique()[0] if row["Empid"] in stopdf["Empid"] else "", axis=1)

    with pd.ExcelWriter(custom.xlresfile, mode="a",if_sheet_exists='replace') as writer:
        findf[["Empid","Empname","mn","Dirug","Division","actBasis","Wagerate","calcBasis","Stop"]].to_excel(writer,sheet_name="בסיס פנסיה",float_format="%.0f",header=["מספר עובד","שם","מנ","דירוג","אגף","בסיס פנסיה בפועל","חלקיות משרה","בסיס מחושב","הפסקה"],index=False)
    #      
